...
usage()
{
echo "obs_self.sh [-g group] [-d dep] [-q queue] [-M cluster] [-s imsize] [-p pixscale] [-c] [-t] obsnum
...
# record submission
python ${base}/bin/track_task.py queue --jobid=${jobid} --task='self-cal' --submission_time=`date +%s` --batch_file=${script} \
