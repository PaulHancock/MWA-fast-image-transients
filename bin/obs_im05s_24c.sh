...
script="${base}queue/im05s_24c_${obsnum}.sh"
...
cat ${base}/bin/im05s_24c_pbcorr.tmpl | sed -e "s:OBSNUM:${obsnum}:g" \
                                 -e "s:IMSIZE:${imsize}:g" \
...
# record submission
python ${base}/bin/track_task.py queue --jobid=${jobid} --task='im05s_24c' --submission_time=`date +%s` --batch_file=${script} \
